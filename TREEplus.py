...
class TREEplus:
    ...
    def __init__(self,
        ...
        #new_df = pd.DataFrame({"Node":node.name, "Node Type":"Parent", "Variable Split":node.split, "n":len(node.indexes),  "Deviance":node.deviance, "Explained Deviance": exp_dev,"Class Probabilities":class_node,"Alpha":[node.alpha],"Beta":[node.beta],"LS Error":[node.error]})
            ...
            if self.method == "LATENT-BUDGET-TREE": #classification only method     #could pass the impurity fn as the method to use in lba [ls or mle] as not used
                ...
                while k < len(ordered_list)-1:
                    ...
                    while n <= max_c: 
                        ...
                        if len(splits) >= max_k: #max k can be a user controlled variable, passed to the TREEplus class , or to growing_tree
                            best_index = between_variance.index(max(between_variance))
                            node.set_beta(betas[best_index])
    ...
    def print_tree(self, all_node = None,leaf= None, filename="TREEplus_tree.png", treefile = "tree.dot", table = False, html = False, print_render = False, visual_pruning = False, merge_leaves = False):
        ...
        if table == True and self.method == "LATENT-BUDGET-TREE": 
            if self.twoing:
                tree_table = pd.DataFrame(columns = ["Node", "Node Type", "Variable Split", "Twoing Classes C1", "Twoing Classes C2", "n", "Deviance","Explained Deviance","Class Probabilities", "Alpha","Beta", "LS Error" ])

            else:
                tree_table = pd.DataFrame(columns = ["Node", "Node Type", "Variable Split", "n", "Deviance","Explained Deviance", "Class Probabilities", "Alpha","Beta","LS Error" ])
            ...
            for node in all_node[n1index:]:
                if int(node.name[1:]) not in leaf_dict.values():
                    ...
                    if self.twoing:
                        ...
                        if self.impurity_fn == "gini":
                            new_df = pd.DataFrame({"Node":node.name, "Node Type":"Parent", "Variable Split":node.split, "n":len(node.indexes), "Deviance":f"{round(node.deviance,2)}", "Explained Deviance": exp_dev, "Class Probabilities":class_node, "Alpha":[node.alpha], "Beta":[node.beta],"LS Error":[node.error]})
                        elif self.impurity_fn == "tau":
                            new_df = pd.DataFrame({"Node":node.name, "Node Type":"Parent", "Variable Split":node.split,"n":len(node.indexes),  "Deviance":f"{round(node.deviance,2)}", "Explained Deviance": exp_dev,"Class Probabilities":class_node, "Alpha":[node.alpha], "Beta":[node.beta],"LS Error":[node.error]})
                        else:
                            new_df = pd.DataFrame({"Node":node.name, "Node Type":"Parent", "Variable Split":node.split, "n":len(node.indexes),  "Deviance":f"{round(node.deviance,2)}", "Explained Deviance": exp_dev,"Class Probabilities":class_node,"Alpha":[node.alpha],"Beta":[node.beta],"LS Error":[node.error]})
                    ...
                    if node_num *2 in leaf_dict.values():
                        ...
                        if self.twoing:
                            if self.impurity_fn == "gini":
                                new_df = pd.DataFrame({"Node":cnode.name, "Node Type":"Child", "Variable Split":None,"Twoing Classes C1":None, "Twoing Classes C2":None, "n":len(cnode.indexes),  "Deviance":f"{round(cnode.deviance,2)}", "Explained Deviance": None,"Class Probabilities":class_node, "Alpha":[cnode.alpha], "Beta":[cnode.beta], "LS Error":[cnode.error]})
                            elif self.impurity_fn == "tau": #tau is only logged if there is a split 
                                new_df = pd.DataFrame({"Node":cnode.name, "Node Type":"Child", "Variable Split":None,"Twoing Classes C1":None, "Twoing Classes C2": None,"n":len(cnode.indexes), "Class Probabilities":class_node, "Deviance":f"{round(cnode.deviance,2)}", "Explained Deviance": None,"Alpha":[cnode.alpha], "Beta":[cnode.beta],"LS Error":[cnode.error]})
                            else:
                                new_df = pd.DataFrame({"Node":cnode.name, "Node Type":"Child", "Variable Split":None,"Twoing Classes C1":None, "Twoing Classes C2":None, "n":len(cnode.indexes),  "Deviance":f"{round(cnode.deviance,2)}", "Explained Deviance": None,"Class Probabilities":class_node,"Alpha":[cnode.alpha],"Beta":[cnode.beta],"LS Error":[cnode.error]})

                        else:
                            if self.impurity_fn == "gini":
                                new_df = pd.DataFrame({"Node":cnode.name, "Node Type":"Child","Variable Split":None, "n":len(cnode.indexes),  "Deviance":f"{round(cnode.deviance,2)}", "Explained Deviance": None,"Class Probabilities":class_node, "Alpha":[cnode.alpha],"Beta":[cnode.beta], "LS Error":[cnode.error]} )
                            elif self.impurity_fn == "tau":  
                                new_df = pd.DataFrame({"Node":cnode.name, "Node Type":"Child", "Variable Split":None,"n":len(cnode.indexes), "Deviance":f"{round(cnode.deviance,2)}", "Explained Deviance": None,"Class Probabilities":class_node, "Alpha":[cnode.alpha], "Beta":[cnode.beta], "LS Error":[cnode.error]})
                            else:
                                new_df = pd.DataFrame({"Node":cnode.name, "Node Type":"Child","Variable Split":None, "n":len(cnode.indexes),  "Deviance":f"{round(cnode.deviance,2)}", "Explained Deviance": None,"Class Probabilities":class_node,"Alpha":[cnode.alpha], "Beta":[cnode.cbeta], "LS Error":[cnode.error]})
                        tree_table = pd.concat([tree_table, new_df], ignore_index=True, sort=False)
        ...
        if table == True:
            if self.twoing:
                tree_table = pd.DataFrame(columns = ["Node", "Node Type", "Variable Split", "Twoing Classes C1", "Twoing Classes C2", "n", "Deviance", "Explained Deviance","Class/Value"])
            else:
                tree_table = pd.DataFrame(columns = ["Node", "Node Type", "Variable Split", "n", "Deviance", "Explained Deviance", "Class/Value"])
            n1node = self.get_key(father_dict, 1)
            n1index = all_node.index(n1node)
            ...
            for node in all_node[n1index:]:
                if int(node.name[1:]) not in leaf_dict.values():
                    ...
                    if self.twoing:
                        if self.impurity_fn == "gini":
                            new_df = pd.DataFrame({"Node":node.name, "Node Type":"Parent", "Variable Split":node.split,"Twoing Classes C1":[self.twoing_c1[node]], "Twoing Classes C2": [self.twoing_c2[node]],"n":len(node.indexes), "Deviance":f"{round(node.deviance,2)}", "Explained Deviance": exp_dev,"Class/Value":[class_node]})
                        elif self.impurity_fn == "tau": 
                            new_df = pd.DataFrame({"Node":node.name, "Node Type":"Parent", "Variable Split":node.split,"Twoing Classes C1":[self.twoing_c1[node]], "Twoing Classes C2": [self.twoing_c2[node]],"n":len(node.indexes),  "Deviance":f"{round(node.deviance,2)}", "Explained Deviance": exp_dev,"Class/Value":[class_node]})
                        else:
                            new_df = pd.DataFrame({"Node":node.name, "Node Type":"Parent", "Variable Split":node.split,"Twoing Classes C1":[self.twoing_c1[node]], "Twoing Classes C2": [self.twoing_c2[node]],"n":len(node.indexes),  "Deviance":f"{round(node.deviance,2)}", "Explained Deviance": exp_dev,"Class/Value":[class_node]})
                        ...
                        if self.impurity_fn == "gini":
                            new_df = pd.DataFrame({"Node":node.name, "Node Type":"Parent", "Variable Split":node.split, "n":len(node.indexes), "Deviance":f"{round(node.deviance,2)}", "Explained Deviance": exp_dev, "Class/Value":[class_node]}) #class node in brackets or need to pass an index 
                        elif self.impurity_fn == "tau": 
                            new_df = pd.DataFrame({"Node":node.name, "Node Type":"Parent", "Variable Split":node.split,"n":len(node.indexes), "Deviance":f"{round(node.deviance,2)}", "Explained Deviance": exp_dev, "Class/Value":[class_node]})
                        else:
                            new_df = pd.DataFrame({"Node":node.name, "Node Type":"Parent", "Variable Split":node.split, "n":len(node.indexes),  "Deviance":f"{round(node.deviance,2)}", "Explained Deviance": exp_dev,"Class/Value":[class_node]})
                    ...
                    if node_num *2 in leaf_dict.values():
                        ...
                        if self.twoing:
                            if self.impurity_fn == "gini":
                                new_df = pd.DataFrame({"Node":cnode.name, "Node Type":"Child", "Variable Split":None,"Twoing Classes C1":None, "Twoing Classes C2": None,"n":len(cnode.indexes),  "Deviance":f"{round(cnode.deviance,2)}", "Explained Deviance": None, "Class/Value":[class_node]})
                            elif self.impurity_fn == "tau": 
                                new_df = pd.DataFrame({"Node":cnode.name, "Node Type":"Child", "Variable Split":None,"Twoing Classes C1":None, "Twoing Classes C2":None,"n":len(cnode.indexes), "Deviance":f"{round(cnode.deviance,2)}", "Explained Deviance": None, "Class/Value":[class_node]})
                            else:
                                new_df = pd.DataFrame({"Node":cnode.name, "Node Type":"Child", "Variable Split":None,"Twoing Classes C1":None, "Twoing Classes C2": None,"n":len(cnode.indexes),  "Deviance":f"{round(cnode.deviance,2)}", "Explained Deviance": None,"Class/Value":[class_node]})
                            ...
                            if self.impurity_fn == "gini":
                                new_df = pd.DataFrame({"Node":cnode.name, "Node Type":"Child","Variable Split":None, "n":len(cnode.indexes),  "Deviance":f"{round(cnode.deviance,2)}", "Explained Deviance": None,"Class/Value":[class_node]} )
                            elif self.impurity_fn == "tau": 
                                new_df = pd.DataFrame({"Node":cnode.name, "Node Type":"Child", "Variable Split":None,"n":len(cnode.indexes), "Deviance":f"{round(cnode.deviance,2)}", "Explained Deviance": None,"Class/Value":[class_node]})
                            else:
                                new_df = pd.DataFrame({"Node":cnode.name, "Node Type":"Child","Variable Split":None, "n":len(cnode.indexes),  "Deviance":f"{round(cnode.deviance,2)}", "Explained Deviance": None,"Class/Value":[class_node]})
...
class Adaboost:
    ...
    def adaboost(self):
        ...
        while iterations < self.weak_learners:
            ...
            model = TREEplus(y, X_num, num_var_1, X_cat, cat_var_1, impurity_fn = self.impurity_fn, problem = self._problem, method = self.method, max_level = self.max_level) 
            ...
            for j in range(len(y_test)): 
                for node in model.get_all_node():  
                    if node.name =="n1":
                        ...
                        model.pred_x(node, d, model.get_all_node(), model.get_leaf()) #appending to a list in TREEplus 
    ...
    def  prediction_fn(self, model, y, X_num_1, num_var_1, X_cat_1, cat_var_1):
        '''Internat prediction function, attaching the model to TREEplus.pred_x'''
        for i in range(len(y)): 
            for node in model.get_all_node():  
                if node.name =="n1":
                    new = []
