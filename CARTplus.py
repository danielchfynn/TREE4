...
# define base class for nodes - not sure if necessary
#class MyBaseClass(object):  # Just a basic base class
#    value = None            # it only brings the node value


class MyNodeClass(NodeMixin):  # Add Node feature #MyBaseClass
    ...
    def bin_split(self, feat, feat_nominal, var_name, soglia):
        '''
        Performs the birnary splitting of teh indices 
        '''
...
class CART:
    ...
    #    #This will work for images maybe, but wont work in general atm 
    #    #df = self.dict_to_dataframe()
    #    dfa = np.array(self.df[node.indexes,:])
    #    print(len(self.df), len(dfa))
    #    #df = df.iloc[node.indexes]
    #    #dfa = dfa[node.indexes,:]
    #    gini = self.gini(node)
    #    tau_list = []
    #    for en, var in enumerate(self.features_names+ self.n_features_names):
    #        firstterm = 0 
    #        #if len(list(set(df[var]))) > 1: #ensuring there is more than one unique element
    #        if len(list(set(dfa[:,int(en)]))) > 1:
    #            #print("hi3",var, Counter(df[var]))
    #            #t = time.time()
    #            for i in list(set(dfa[:,int(en)])):
    #                #df2 = df.loc[df[var]==i]
    #                #print("time1", time.time() - t, df2.shape)
    #                df2a = dfa[dfa[:,int(en)] == i,: ]
    #                #print("time1a", time.time() - t, df2a.shape, list(set(df2a[:,-1])))
    #                #for j in list(set(df2["y"])):
    #                #    firstterm += (len(df2.loc[df2["y"] == j])/len(df2))**2 * len(df2)/len(df) 
    #                #    print("time2", time.time() - t)
    #                for j in list(set(df2a[:,-1])):
    #                    firstterm += (len(df2a[:,-1] == j)/len(df2a))**2 * len(df2a)/len(self.df) 
    #                    #print("time2a", time.time() - t)
    #            tau_list.append(((firstterm - gini) / (1-gini), var))
    #            #print("taulist", tau_list)
    ...
    def pearson_ordering(self, node):
        ...
        for en, var in enumerate(self.features_names+ self.n_features_names):  #reworked a bit to work with images
            wss = 0 
            #print(dfa.dtype, type(dfa), j, var, dfa[0:5,j])
            if len(list(set(dfa[:,int(en)]))) > 1:
                for i in list(set(dfa[:,int(en)])):
                    #df2 = df.loc[df[var]==i]
                    df2a = dfa[dfa[:,int(en)] == i,: ]
    ...
    def midway_points(self, var, node):
        ...
        sett = list(set(self.features[str(var)][node.indexes]))
        sett.sort()
        for i in range(len(sett)-1):
            midway_points.append((sett[i]+sett[i+1])/2)
    ...
    def __node_search_split(self,node:MyNodeClass, max_k, combination_split, max_c):
       ...
       Attenzione: questo è un metodo privato non chiamabile a di fuori della classe.
        ...
        #print("Counter", Counter(self.y), self.y.index )
            ...
            if self.method == "LATENT-BUDGET-TREE": #classification only method     #could pass the impurity fn as the method to use in lba [ls or mle] as not used
                ...
                while k < len(ordered_list)-1:
                    ...
                    while n <= max_c: 
                        ...
                        if len(splits) >= max_k: #max k can be a user controlled variable, passed to the CART class , or to growing_tree
                            best_index = between_variance.index(max(between_variance))
                            node.set_beta(betas[best_index])
        ...
        if self.twoing:
            ...
            if self.problem == "classifier":
                ...
                for i in distinct_values: 
                    ...
                    #print("t2", distinct_values.index(i),time.time() - t)
            ...
            for inode in self.bigtree:
                if inode not in self.father:
                    if self.problem == 'regression':
                        ...
                        #print(inode.name, self.RSS(self.y[inode.indexes]), len(self.y[inode.indexes]) )
                    elif self.problem == 'classifier':
        ...
        for i in new_dict:
            if len(new_dict[i][1]) == 2:
                ...
                for j in new_dict[i][1]:
                    #print(j.get_value(cart.y, cart.problem))
    ...
    def pruning(self, features_test, n_features_test, y_test, 
        ...
        while len(all_node) >=3: #have changed this to 1 without just leaving root node, could just append to end of list if wanted 
            ...
            if self.problem =='regression':
                ...
                missclass1 = 0   #is this for a 4 class problem only? where the classes are numbered 1 - 4 not sure if reliable, i think set up for lbt problem TODO
                class1 = 0
                missclass2 = 0 
                class2 = 0
                missclass3 = 0 
        ...
        if self.problem =='regression':
            ...
            for i in pruned_trees:
                if i[0] == key_min:
                    tree_table = self.print_tree(i[1], i[2], png_name,dot_name, table = table, html = html, print_render= print_render, merge_leaves = merge_leaves)
            ...
            missg = [i/ len(self.y) for i in miss] 
            misstest = [i/len(y_test) for i in list(leaves_miss.values())]         
            self.graph_results(leaves_for_prune,missg,"Training Set", list(leaves_miss.keys()),misstest,"Testing Set") #x1, y1, label1, x2, y2, label2 #list(leaves_miss.values())/len(y_test)
    ...
    def cut_tree(self,how_many_leaves:int):
        ...
        while len(self.leaf) > how_many_leaves: #was != 
    ...
    def print_tree(self, all_node = None,leaf= None, filename="CART_tree.png", treefile = "tree.dot", table = False, html = False, print_render = False, visual_pruning = False, merge_leaves = False):
        ...
        if visual_pruning:   #problem if the split with the highest purity gain is not the first, aka, lbt
            ...
            #    if self.get_key(father_dict, father_list[i]).node_prop:
            #        node_prop_dict[father_list[i]] = self.get_key(father_dict, father_list[i]).node_prop
            ...
            #position[1] = [0 , 2*position[2][1]] #down tree correction with initial node
        ...
        for node in all_node:
            if self.problem == "regression":
                node.deviance = round(len(self.y[node.indexes])* self.RSS(self.y[node.indexes]),2)
            else:
                node.deviance = round(self.deviance_cat2(node), 2)

        if table == True and self.method == "LATENT-BUDGET-TREE": 
            if self.twoing:
                tree_table = pd.DataFrame(columns = ["Node", "Node Type", "Variable Split", "Twoing Classes C1", "Twoing Classes C2", "n", "Impurity Value", "Deviance","Explained Deviance","Class Probabilities", "Alpha","Beta", "LS Error" ])

            else:
                tree_table = pd.DataFrame(columns = ["Node", "Node Type", "Variable Split", "n", "Impurity Value", "Deviance","Explained Deviance", "Class Probabilities", "Alpha","Beta","LS Error" ])
            ...
            for node in all_node[n1index:]:
                if int(node.name[1:]) not in leaf_dict.values():
                    ...
                    if self.twoing:
                        if self.impurity_fn == "gini":
                            new_df = pd.DataFrame({"Node":node.name, "Node Type":"Parent", "Variable Split":node.split,"Twoing Classes C1":[self.twoing_c1[node]], "Twoing Classes C2": [self.twoing_c2[node]],"n":len(node.indexes), "Impurity Value":f"{round(self.impur(node, display = True),2)}", "Deviance":node.deviance, "Explained Deviance": exp_dev, "Class Probabilities":class_node, "Alpha":[node.alpha], "Beta":[node.beta],"LS Error":[node.error] })
                        elif self.impurity_fn == "tau":
                            new_df = pd.DataFrame({"Node":node.name, "Node Type":"Parent", "Variable Split":node.split,"Twoing Classes C1":[self.twoing_c1[node]], "Twoing Classes C2": [self.twoing_c2[node]],"n":len(node.indexes), "Impurity Value":f"{round(node.value_soglia_split[0][2],2)}", "Deviance":node.deviance, "Explained Deviance": exp_dev, "Class Probabilities":class_node, "Alpha":[node.alpha], "Beta":[node.beta],"LS Error":[node.error]})
                        else:
                            new_df = pd.DataFrame({"Node":node.name, "Node Type":"Parent", "Variable Split":node.split,"Twoing Classes C1":[self.twoing_c1[node]], "Twoing Classes C2": [self.twoing_c2[node]],"n":len(node.indexes), "Impurity Value":f"{round(self.impur(node),2)}", "Deviance":node.deviance, "Explained Deviance": exp_dev,"Class Probabilities":class_node,"Alpha":[node.alpha],"Beta":[node.beta],"LS Error":[node.error]})

                    else:
                        if self.impurity_fn == "gini":
                            new_df = pd.DataFrame({"Node":node.name, "Node Type":"Parent", "Variable Split":node.split, "n":len(node.indexes), "Impurity Value":f"{round(self.impur(node, display = True),2)}", "Deviance":node.deviance, "Explained Deviance": exp_dev, "Class Probabilities":class_node, "Alpha":[node.alpha], "Beta":[node.beta],"LS Error":[node.error]})
                        elif self.impurity_fn == "tau":
                            new_df = pd.DataFrame({"Node":node.name, "Node Type":"Parent", "Variable Split":node.split,"n":len(node.indexes), "Impurity Value":f"{round(node.value_soglia_split[0][2],2)}", "Deviance":node.deviance, "Explained Deviance": exp_dev,"Class Probabilities":class_node, "Alpha":[node.alpha], "Beta":[node.beta],"LS Error":[node.error]})
                        else:
                            new_df = pd.DataFrame({"Node":node.name, "Node Type":"Parent", "Variable Split":node.split, "n":len(node.indexes), "Impurity Value":f"{round(self.impur(node),2)}", "Deviance":node.deviance, "Explained Deviance": exp_dev,"Class Probabilities":class_node,"Alpha":[node.alpha],"Beta":[node.beta],"LS Error":[node.error]})
                    ...
                    if node_num *2 in leaf_dict.values():
                        ...
                        if self.twoing:
                            if self.impurity_fn == "gini":
                                new_df = pd.DataFrame({"Node":cnode.name, "Node Type":"Child", "Variable Split":None,"Twoing Classes C1":None, "Twoing Classes C2":None, "n":len(cnode.indexes), "Impurity Value":f"{round(self.impur(cnode, display = True),2)}", "Deviance":cnode.deviance, "Explained Deviance": None,"Class Probabilities":class_node, "Alpha":[cnode.alpha], "Beta":[cnode.beta], "LS Error":[cnode.error]})
                            elif self.impurity_fn == "tau": #tau is only logged if there is a split 
                                new_df = pd.DataFrame({"Node":cnode.name, "Node Type":"Child", "Variable Split":None,"Twoing Classes C1":None, "Twoing Classes C2": None,"n":len(cnode.indexes), "Impurity Value":None, "Class Probabilities":class_node, "Deviance":cnode.deviance, "Explained Deviance": None,"Alpha":[cnode.alpha], "Beta":[cnode.beta],"LS Error":[cnode.error]})
                            else:
                                new_df = pd.DataFrame({"Node":cnode.name, "Node Type":"Child", "Variable Split":None,"Twoing Classes C1":None, "Twoing Classes C2":None, "n":len(cnode.indexes), "Impurity Value":f"{round(self.impur(cnode),2)}", "Deviance":cnode.deviance, "Explained Deviance": None,"Class Probabilities":class_node,"Alpha":[cnode.alpha],"Beta":[cnode.beta],"LS Error":[cnode.error]})

                        else:
                            if self.impurity_fn == "gini":
                                new_df = pd.DataFrame({"Node":cnode.name, "Node Type":"Child","Variable Split":None, "n":len(cnode.indexes),  "Impurity Value":f"{round(self.impur(cnode, display = True),2)}","Deviance":cnode.deviance, "Explained Deviance": None,"Class Probabilities":class_node, "Alpha":[cnode.alpha],"Beta":[cnode.beta], "LS Error":[cnode.error]} )
                            elif self.impurity_fn == "tau":  
                                new_df = pd.DataFrame({"Node":cnode.name, "Node Type":"Child", "Variable Split":None,"n":len(cnode.indexes), "Impurity Value":None, "Deviance":cnode.deviance, "Explained Deviance": None,"Class Probabilities":class_node, "Alpha":[cnode.alpha], "Beta":[cnode.beta], "LS Error":[cnode.error]})
                            else:
                                new_df = pd.DataFrame({"Node":cnode.name, "Node Type":"Child","Variable Split":None, "n":len(cnode.indexes),  "Impurity Value":f"{round(self.impur(cnode),2)}","Deviance":cnode.deviance, "Explained Deviance": None,"Class Probabilities":class_node,"Alpha":[cnode.alpha], "Beta":[cnode.cbeta], "LS Error":[cnode.error]})
                        tree_table = pd.concat([tree_table, new_df], ignore_index=True, sort=False)
                    if node_num *2+1 in leaf_dict.values():
                        ...
                        if self.twoing:
                            if self.impurity_fn == "gini":
                                ...
                                new_df = pd.DataFrame({"Node":cnode.name, "Node Type":"Child", "Variable Split":None,"Twoing Classes C1":None, "Twoing Classes C2": None,"n":len(cnode.indexes), "Impurity Value":None, "Deviance":cnode.deviance, "Explained Deviance": None,"Class Probabilities":class_node, "Alpha":[cnode.alpha], "Beta":[cnode.beta], "LS Error":[cnode.error]})
                            else:
                                new_df = pd.DataFrame({"Node":cnode.name, "Node Type":"Child", "Variable Split":None,"Twoing Classes C1":None, "Twoing Classes C2":None, "n":len(cnode.indexes), "Impurity Value":f"{round(self.impur(cnode),2)}", "Deviance":cnode.deviance, "Explained Deviance": None,"Class Probabilities":class_node,"Alpha":[cnode.alpha],"Beta":[cnode.beta], "LS Error":[cnode.error]})
                        else:
                            if self.impurity_fn == "gini":
                                ...
                                new_df = pd.DataFrame({"Node":cnode.name, "Node Type":"Child","Variable Split":None, "n":len(cnode.indexes),  "Impurity Value":f"{round(self.impur(cnode),2)}","Deviance":cnode.deviance, "Explained Deviance": None,"Class Probabilities":class_node, "Alpha":[cnode.alpha],"Beta":[cnode.beta], "LS Error":[cnode.error]} )
                        tree_table = pd.concat([tree_table, new_df], ignore_index=True, sort=False)
        ...
        if table == True:
            if self.twoing:
                tree_table = pd.DataFrame(columns = ["Node", "Node Type", "Variable Split", "Twoing Classes C1", "Twoing Classes C2", "n", "Impurity Value", "Deviance", "Explained Deviance","Class/Value"])
            else:
                tree_table = pd.DataFrame(columns = ["Node", "Node Type", "Variable Split", "n", "Impurity Value", "Deviance", "Explained Deviance", "Class/Value"])
            n1node = self.get_key(father_dict, 1)
            n1index = all_node.index(n1node)
            ...
            for node in all_node[n1index:]:
                if int(node.name[1:]) not in leaf_dict.values():
                    ...
                    if self.twoing:
                        if self.impurity_fn == "gini":
                            new_df = pd.DataFrame({"Node":node.name, "Node Type":"Parent", "Variable Split":node.split,"Twoing Classes C1":[self.twoing_c1[node]], "Twoing Classes C2": [self.twoing_c2[node]],"n":len(node.indexes), "Impurity Value":f"{round(self.impur(node, display = True),2)}", "Deviance":node.deviance, "Explained Deviance": exp_dev,"Class/Value":[class_node]})
                        elif self.impurity_fn == "tau": 
                            new_df = pd.DataFrame({"Node":node.name, "Node Type":"Parent", "Variable Split":node.split,"Twoing Classes C1":[self.twoing_c1[node]], "Twoing Classes C2": [self.twoing_c2[node]],"n":len(node.indexes), "Impurity Value":f"{round(node.value_soglia_split[0][2],2)}", "Deviance":node.deviance, "Explained Deviance": exp_dev,"Class/Value":[class_node]})
                        else:
                            new_df = pd.DataFrame({"Node":node.name, "Node Type":"Parent", "Variable Split":node.split,"Twoing Classes C1":[self.twoing_c1[node]], "Twoing Classes C2": [self.twoing_c2[node]],"n":len(node.indexes), "Impurity Value":f"{round(self.impur(node),2)}", "Deviance":node.deviance, "Explained Deviance": exp_dev,"Class/Value":[class_node]})

                    else:
                        if self.impurity_fn == "gini":
                            new_df = pd.DataFrame({"Node":node.name, "Node Type":"Parent", "Variable Split":node.split, "n":len(node.indexes), "Impurity Value":f"{round(self.impur(node, display = True),2)}", "Deviance":node.deviance, "Explained Deviance": exp_dev, "Class/Value":[class_node]}) #class node in brackets or need to pass an index 
                        elif self.impurity_fn == "tau": 
                            new_df = pd.DataFrame({"Node":node.name, "Node Type":"Parent", "Variable Split":node.split,"n":len(node.indexes), "Impurity Value":f"{round(node.value_soglia_split[0][2],2)}","Deviance":node.deviance, "Explained Deviance": exp_dev, "Class/Value":[class_node]})
                        else:
                            new_df = pd.DataFrame({"Node":node.name, "Node Type":"Parent", "Variable Split":node.split, "n":len(node.indexes), "Impurity Value":f"{round(self.impur(node),2)}", "Deviance":node.deviance, "Explained Deviance": exp_dev,"Class/Value":[class_node]})
                    ...
                    if node_num *2 in leaf_dict.values():
                        ...
                        if self.twoing:
                            if self.impurity_fn == "gini":
                                new_df = pd.DataFrame({"Node":cnode.name, "Node Type":"Child", "Variable Split":None,"Twoing Classes C1":None, "Twoing Classes C2": None,"n":len(cnode.indexes), "Impurity Value":f"{round(self.impur(cnode, display = True),2)}", "Deviance":cnode.deviance, "Explained Deviance": None, "Class/Value":[class_node]})
                            elif self.impurity_fn == "tau": 
                                new_df = pd.DataFrame({"Node":cnode.name, "Node Type":"Child", "Variable Split":None,"Twoing Classes C1":None, "Twoing Classes C2":None,"n":len(cnode.indexes), "Impurity Value":None, "Deviance":cnode.deviance, "Explained Deviance": None, "Class/Value":[class_node]})
                            else:
                                new_df = pd.DataFrame({"Node":cnode.name, "Node Type":"Child", "Variable Split":None,"Twoing Classes C1":None, "Twoing Classes C2": None,"n":len(cnode.indexes), "Impurity Value":f"{round(self.impur(cnode),2)}", "Deviance":cnode.deviance, "Explained Deviance": None,"Class/Value":[class_node]})
                        else:
                            if self.impurity_fn == "gini":
                                new_df = pd.DataFrame({"Node":cnode.name, "Node Type":"Child","Variable Split":None, "n":len(cnode.indexes),  "Impurity Value":f"{round(self.impur(cnode, display = True),2)}","Deviance":cnode.deviance, "Explained Deviance": None,"Class/Value":[class_node]} )
                            elif self.impurity_fn == "tau": 
                                new_df = pd.DataFrame({"Node":cnode.name, "Node Type":"Child", "Variable Split":None,"n":len(cnode.indexes), "Impurity Value":None, "Deviance":cnode.deviance, "Explained Deviance": None,"Class/Value":[class_node]})
                            else:
                                new_df = pd.DataFrame({"Node":cnode.name, "Node Type":"Child","Variable Split":None, "n":len(cnode.indexes),  "Impurity Value":f"{round(self.impur(cnode),2)}","Deviance":cnode.deviance, "Explained Deviance": None,"Class/Value":[class_node]})
    ...
    def pred_x(self,node, x, all_node, leaves): #-> tree :
        ...
        if node in leaves:                            #Provides the final output for the predicted node
            ...
            if self.combination_split: #not 100% functional, issues with errors in x values, and appears every split leads to a true?
                ...
                #print("eval", node.split, x)
                new_node = self.get_key(node_dict, int(node.name[1:])*2+1)
                self.pred_x(new_node, x, all_node, leaves) # go to the right child
            else:
                #print("eval2", node.split, x)
                new_node = self.get_key(node_dict, int(node.name[1:])*2)
                self.pred_x(new_node, x, all_node, leaves) # go to the left child
...
class Adaboost:
    ...
    def new_df(self):
        ...
        for i in range(len(random_list)):
            for j in range(len(self.df["weights"])):
                if random_list[i] < self.df["cum_sum"][j]:
    ...
    def adaboost(self):
        ...
        while iterations < self.weak_learners:
            ...
            model = CART(y, X_num, num_var_1, X_cat, cat_var_1, impurity_fn = self.impurity_fn, problem = self._problem, method = self.method, max_level = self.max_level) 
    ...
    def  prediction_fn(self, model, y, X_num_1, num_var_1, X_cat_1, cat_var_1):
        '''Internat prediction function, attaching the model to CART.pred_x'''
        for i in range(len(y)): 
            for node in model.get_all_node():  
                if node.name =="n1":
                    new = []
...
class k_folds():
    ...
    def __init__(self, 
        ...
        for n, i in enumerate(self.folds):
            ...
            my_tree = MyNodeClass('n1', ind) 
            model = CART(y, features, features_names, n_features, n_features_names, impurity_fn = self.impurity_fn, problem = self.problem, method = self.method, max_level = self.max_level) 
            model.growing_tree(my_tree)
